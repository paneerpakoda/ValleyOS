...
- **Content type richness.** Should V0-2 JSON include `category` / `timeHint` / `deadline` / `bundleContext` / `wikiUrl` now, or add on third use per CLAUDE.md §9? See closed [PR #8](https://github.com/paneerpakoda/ValleyOS/pull/8) (branch `feature/early-test-proto`) for the rich-types alternative.
